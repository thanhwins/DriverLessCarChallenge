...
bool SignDetection::objectLabeling(std::vector<cv::Rect> &boxes, std::vector<int> &labels, cv::Mat &depth, cv::Mat &color, ushort &low_th, ushort &high_th, int min_pts , int max_pts , ushort min_w, ushort max_w , double ratio )
{
	ushort depth_val;
	ushort hole = 0, tmp_v;
	cv::Point p;
	...
	//depth_val = clone_depth.at<ushort>(320,240);
	...
	for (int y = 1; y < clone_depth.rows; y++)
	{
		for (int x = 1; x < clone_depth.cols - 1; x++)
		{
			...
			if (tmp_v != hole)
			{
				this -> floodFill(reg, clone_depth, p, 1);
				...
				if (p1.y - 20 >= 0) p1.y -= 20;
				if (p2.x + 20 < clone_depth.cols) p2.x +=20;
				if (p2.y + 20 < clone_depth.rows) p2.y += 20;
				cv::Mat object = clone(cv::Rect(p1.x, p1.y, p2.x - p1.x, p2.y - p1.y));
				cv::Mat binary;
